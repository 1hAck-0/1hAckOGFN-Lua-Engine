...
public:
	...
	void StartCapturingFinalPixels(struct FName StreamName);
	void ResolveBuffer(class UTexture* Buffer, struct FName BufferName);
	void PushBufferToStream(class UTexture* Buffer, struct FName StreamName);
	void OnWarmUp();
	void OnTick();
	void OnStartCapture();
	bool OnSetup();
	...
	void BindToStream(struct FName StreamName);
};
...
public:
	...
	void WriteImageToDisk(struct FCapturedPixels PixelData, struct FName StreamName, struct FFrameMetrics FrameMetrics, bool bCopyImageData);
	struct FString GenerateFilenameForCurrentFrame();
	struct FString GenerateFilenameForBuffer(class UTexture* Buffer, struct FName StreamName);
};
